...
Uncover the appropriate plaintext and submit the flags below! Do you know what all these ones and zeros mean? (1 point)
...
At first glance, these are binary values separated by spaces. Using CyberChef "From Binary", I decode this to: 
```
...
These look like triplets, a classic ASCII octal pattern. Using dcode.fr ASCII Converter "Octal to Text" gives: 
```
...
These numbers are in standard decimal ASCII form. Converting again using dcode.fr ASCII Code:
```
...
Pairs of numbers with occasional letters? That’s hexadecimal.
Decoded in CyberChef using "From Hex":
```
...
Uppercase letters and digits 2–7 — that’s a dead giveaway for Base32.
Decoded output:
```
...
The odd mix of symbols and letters indicates Base45:
```
...
### 7 Base64
The = padding at the end is a giveaway for Base64.
```
...
### 9 Base92
I tried a few differnt things with not a lot of luck at first when copying the text as is. 
Ended up trying base92 decode on each line separately:
```
...
### 10 Base65536
I remember seeing text like this from a past CTF and found it was Base65536:
```
